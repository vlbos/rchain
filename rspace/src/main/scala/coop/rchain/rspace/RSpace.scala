...
class RSpace[F[_], C, P, E, A, R, K] private[rspace] (
  ...
  private[this] val produceTimer         = Kamon.timer(MetricsSource + ".produce-time")
  private[this] val produceCounter       = Kamon.counter(MetricsSource + ".produce")
  private[this] val produceFailedCounter = Kamon.counter(MetricsSource + ".produce-failed")
      ...
      syncF
        .delay {
          Kamon.withSpan(produceSpan.start(), finishSpan = true) {
            ...
            val result = produceLock(channel) {
              ...
              def extractProduceCandidate(
                ...
                groupedChannels match {
                  ...
                  case channels :: remaining =>
                     ...
                     */
                    span.mark("before-channel-to-indexed-data")
                    val channelToIndexedData: Map[C, Seq[(Datum[A], Int)]] = channels.map {
                      (c: C) =>
        ...
        .onError { case _ => Sync[F].delay(produceFailedCounter.increment()) }
    }
